...
* **Managed updates:** Settings default to `X:\Apps\date-formatter.exe`. The path can also point to a folder or directly to an EXE on a mapped drive or UNC share. Windows builds compare EXE metadata, stage newer builds locally, verify SHA-256, then use a restart helper to replace the running EXE.

### macOS public release

* **Asset:** `date-formatter-v0.2.15-macos-arm64.zip`
...
* **Assets:** `date-formatter-v0.2.15-linux-amd64.deb`, `date-formatter-v0.2.15-linux-x86_64.rpm`, and `date-formatter-v0.2.15-linux-amd64.tar.gz`
...
Older standalone batch launchers for single-purpose converters have been removed from `prod/`. Use `date-formatter-gui.bat` for legacy Python deployments.
...
| Surface | Location |
|---------|----------|
| Wails settings | Windows: `%APPDATA%\date-formatter\settings.json`; macOS: `~/Library/Application Support/date-formatter/settings.json`; Linux: `~/.config/date-formatter/settings.json`. |
