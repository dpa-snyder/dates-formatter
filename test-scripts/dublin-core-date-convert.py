...
def convert_date_pattern(date_str):
    try:
        ...
        if re.match(r'\d{4}-\d{2}', date_str):
            ...
            return f"{month:02d}/01/{year} - {month:02d}/{get_last_day_of_month(year, month)}/{year}"
        ...
        if re.match(r'\d{4}/\d{4}-\d{2}', date_str):
            ...
            if '-' in end_part:
                ...
                return f"01/01/{start_year} - {end_month:02d}/{get_last_day_of_month(end_year, end_month)}/{end_year}"

        if re.match(r'\d{4}-\d{2}/\d{4}', date_str):
            start_part, end_year = date_str.split('/')
            start_year, start_month = map(int, start_part.split('-'))
            return f"{start_month:02d}/01/{start_year} - {start_month:02d}/{get_last_day_of_month(start_year, start_month)}/{end_year}"
...
def process_dataframe(df, column):
    ...
    df[new_column_name] = df[column].apply(lambda x: convert_date_pattern(str(x)) if pd.notna(x) else 'undated')
...
# TODO: move new column to the right of the original column
# TODO: resize tkinter window
# TODO: add headers for leading zeros
