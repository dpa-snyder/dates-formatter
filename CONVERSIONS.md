...
Per-mode input/output spec in parser order. First match wins. See `MANUAL.md` and `user-manual.html` for the user-facing version of this document.
...
| 5 | `MonthName D, YYYY - MonthName D, YYYY` | `June 1, 1962 - August 5, 1964` | `06/01/1962 - 08/05/1964` | |
| 6 | Same with comma and dash variant | `Jun 1, 1962 - Aug 5, 1964` | `06/01/1962 - 08/05/1964` | |
...
| 17 | Decade shorthand `YYYY-YY` (`1962-65`) | `1962-65` | `01/01/1962 - 12/31/1965` | |
...
| 22 | Range containing `??` or `00` | `06/??/1962 - 08/??/1962` | `06/01/1962 - 08/31/1962` | |
...
2. `ensure_chronological_order` swaps any range that came out reversed (`12/31/1965 - 01/01/1962`).
...
| 15 | ISO range with `to` literal (`... TO ...`) | `1962-06-05 TO 1965-08-12` | recurses to `06/05/1962 - 08/12/1965` |
...
Dublin Core mode does not apply `convert_strange_named_ranges` or chronological order swap. Check column flags any output that does not match `MM/DD/YYYY`, range, or `undated`.
...
| Any | empty | empty |
| Any | unknown shape (`12A`, `W2X`) | unchanged (no crash) |

All other columns (Folder Number, Sequential Box Number, Title, etc.) are read as text and saved as text. Leading zeros are preserved and no numeric coercion occurs.
