...
|----|-------|------------|-------|-------|
| T-001 | 1 | 4 | Validate `v0.2.13` release in real Windows, macOS, and Linux environments | Smoke-test public Windows `date-formatter.exe`, macOS arm64 zip, Linux `.deb`, Linux `.rpm`, and Linux tarball with `9200-W22.xlsx`. Confirm browser warnings, SmartScreen, Gatekeeper behavior, Linux runtime dependencies, YY prefix, overwrite/copy output, date-column auto-detection, open file/folder actions, and Windows shared-folder update/restart flow. Enterprise signed builds may behave differently. |
